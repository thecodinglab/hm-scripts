...
def step(i, x, funcs, jacobi_x0, k_max):
    f_x = np.array([
        f(x) for f in funcs
    ])

    # TODO: ability to choose solver?
    gamma_x = np.linalg.solve(jacobi_x0, -f_x)
...
jacobi_x0 = np.array([
    df(x0) for df in jacobi
])

x_prev = x0
i = 0

x = step(i, x0, funcs, jacobi_x0, k_max)

while not criterion({'i': i, 'max': term_max, 'x': x, 'x_prev': x_prev, 'f_x': np.array([f(x) for f in funcs])}):
    x_prev = x
    x = step(i, x, funcs, jacobi_x0, k_max)
    i += 1
